...
function useWallet() {
  ...
  async function updateAccount(
    ...
    try {
      ...
      balance = await lootContract.balanceOf(address);
  ...
  async function login(newModal: Web3Modal) {
    try {
      ...
      if (address) {
        await updateAccount(provider, address, lootContract, signer);
      }

      if (rawProvider) {
        rawProvider.on("accountsChanged", async (accounts: string[]) => {
          if (accounts?.length > 0) {
            await updateAccount(provider, accounts[0], lootContract, signer);
          }
        });
